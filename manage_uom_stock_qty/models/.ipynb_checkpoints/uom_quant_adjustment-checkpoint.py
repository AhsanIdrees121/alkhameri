...
class UomQuantAdjustment(models.Model):
    ...
    def exit_lot_number_domain(self):
        ...
        if self.converted_uom_id and self.exiting_lot and self.converted_uom_id.id == self.selected_uom_id.id:
            ...
            lot_number = self.env['stock.production.lot'].search([('product_id', '=', self.product_id.id),
    ...
    def confirm_action(self):
        ...
        for line in self.uom_adjustment_line:
            valuation_line = all_line_ids.search([('inventory_id', '=', stock_inventory_id.id),
                                                  ('prod_lot_id', '=', line.lot_id.id),
                                                  ('product_uom_id', '=', line.product_uom_id.id),
                                                  ('location_id.usage', '=', 'internal')])
            valuation_line.product_qty = valuation_line.product_qty - line.counted

        if self.exiting_lot:
            valuation_line = all_line_ids.search([('inventory_id', '=', stock_inventory_id.id),
                                                  ('prod_lot_id', '=', self.exist_lot_id.id),
            ...
            lot_id = self.env['stock.lot'].create({
                ...
                'product_qty_2': self.changing_value,
                'product_uom_id': self.converted_uom_id.id,
                'uom_id_2': self.converted_uom_id.id,
            })
            new_line = self.env['stock.inventory.line'].create({
